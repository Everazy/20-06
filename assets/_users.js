...
// ── Cair poin referral setelah penerima order pertama ───────────────────────
// Dipanggil dari payment-callback.js setiap kali order berhasil.
// Hanya berjalan sekali: saat referralPendingPayout masih true.
export async function maybePayoutPendingReferral({ baseUrl, fbApiKey, buyerUid }) {
  if (!buyerUid) return;
  try {
    ...
    // Hanya lanjut kalau payout masih pending
    if (f.referralPendingPayout?.booleanValue !== true) return;
    const referrerUid = f.referredBy?.stringValue;
    if (!referrerUid) return;

    // Tandai payout sudah dilakukan (lakukan dulu sebelum award, cegah double-payout)
    await fetch(`${userUrl}&updateMask.fieldPaths=referralPendingPayout`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ fields: { referralPendingPayout: { booleanValue: false } } })
    });

    // Beri poin ke pemberi referral
    await awardReferralPoints({ baseUrl, fbApiKey, referrerUid });
    console.log(`✅ Referral payout: ${POINTS_REFERRAL} poin ke ${referrerUid} (triggered by order dari ${buyerUid})`);
  } catch (e) {
    console.warn('maybePayoutPendingReferral gagal (diabaikan):', e.message);
  }
}
